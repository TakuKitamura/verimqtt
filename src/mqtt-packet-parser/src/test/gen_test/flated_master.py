...
def explore_parsed_typedef_structs(explore_target, origin_structs):
    ...
    for data_name in explore_target:
        ...
        if (data_type in origin_structs.keys()):
            explored[data_name] = explore_parsed_typedef_structs(
                origin_structs[data_type], origin_structs)
        else:
            explored[data_name] = explore_target[data_name]
    return explored


def get_typedef_struct_json(file_path):
            ...
            if (stripped_line.startswith('typedef struct ')):
                ...
                if (find_typedef == True and  # {}内のテキスト
                    ...
                    for data in data_contents:
                        ...
                        data_type = (
                            splitted_line[0] + ('*' * asterisk_count)).replace('C_String_t', 'const char*')
    ...
    for struct_name in origin_structs:
        ...
        explored = explore_parsed_typedef_structs(
            explore_target, origin_structs)
        extentioned_structs[struct_name] = explored

    structs_data_map_json = json.dumps(extentioned_structs['parse_result'])
    return structs_data_map_json
